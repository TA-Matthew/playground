...
export function SecretUnlock({ onToggleUnlock, title = 'Facilitator unlock' }: Props) {
  return (
    <button
      ...
      className="hidden md:block fixed bottom-4 left-4 z-50 h-11 w-11 cursor-pointer rounded-2xl border border-stone-200/60 bg-white/80 opacity-50 shadow-sm backdrop-blur-sm hover:opacity-90 hover:ring-2 hover:ring-amber-200/80"
      title={title}
      aria-label={title}
    />
  )
