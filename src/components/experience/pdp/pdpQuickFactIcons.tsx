...
/** Matches {@link PdpViatorIconRail} label line. */
export const PDP_QUICK_FACT_TEXT_CLASS =
  'text-base font-normal leading-[1.5] text-[#000000]'

export function formatQuickFactLine(it: IconRailItem): string {
  return it.label ? `${it.label}: ${it.value}` : it.value
}

export function PdpQuickFactLineText({ item }: { item: IconRailItem }) {
  return (
    <span className={PDP_QUICK_FACT_TEXT_CLASS}>
      {item.label ? (
        <>
...
/** Shared quick-fact icons (clock / mobile / language) — same SVG as Viator rail. */
...
export const VIATOR_QUICK_FACT_ICON_FILL = '#008768'

export function PdpQuickFactClockIcon({
...
export function PdpQuickFactMobileIcon({
  className = 'size-6 shrink-0',
  fill = 'currentColor',
}: IconProps) {
  return (
...
export function PdpQuickFactLanguageIcon({
  className = 'size-6 shrink-0',
  fill = 'currentColor',
}: IconProps) {
  return (
...
export function PdpQuickFactRailIcon({
  kind,
  ...
  const props = { className, fill }
  if (kind === 'clock') return <PdpQuickFactClockIcon {...props} />
  if (kind === 'mobile') return <PdpQuickFactMobileIcon {...props} />
  return <PdpQuickFactLanguageIcon {...props} />
}
